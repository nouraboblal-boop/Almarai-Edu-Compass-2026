...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-NnUdLB0-.js");
...
function getResponseLinkHeaderEntries(opts) {
  if (!opts.filter) return opts.entries.map((entry2) => entry2.link);
  try {
    const links = [];
    for (const entry2 of opts.entries) if (opts.filter(entry2)) links.push(entry2.link);
    return links;
  } catch (err) {
...
function appendResponseLinkHeaders(opts) {
  if (!opts.filter) {
    for (const entry2 of opts.entries) opts.responseHeaders.append("Link", entry2.link);
    return;
  }
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-D-X44q6b.js"),
...
const entry = createStartHandler(defaultStreamHandler);
...
const server = createServerEntry({ fetch: entry });
export {
  createServerEntry,
  server as default
};
